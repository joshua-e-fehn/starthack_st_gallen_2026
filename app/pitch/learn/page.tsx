...
// Find the asset-classes lesson from the shared data
const lesson = LESSONS.find((l) => l.id === "asset-classes")!

export default function PitchLearnPage() {
  ...
  const goNext = useCallback(() => {
    if (isLastSlide) {
      // Redirect to homepage after completing the lesson
      router.push("/")
    } else {
      setDirection(1)
      setCurrentSlide((prev) => Math.min(prev + 1, totalSlides - 1))
    }
  }, [isLastSlide, totalSlides, router])
